...
The deterministic concept pass is repo-specific rather than a fixed keyword list. It scores candidate concepts from file paths, exported names, and symbols, applies configured domain vocabulary mappings and signal weights when present, then connects concepts back to owning nodes and related files.

The deterministic subsystem pass adds first-level human navigation nodes beneath `project.intent`. It looks for configured subsystem patterns first, then built-in evidence such as UI packages, core engines, CLI/API surfaces, goal or mission automation, memory/validation/log helpers, documentation/examples, packaging/adapters, and tests/quality gates. It only creates a subsystem when there is matching evidence; a repo without app evidence should not get an app node unless the project config explicitly defines one. If no strong subsystem pattern exists, it falls back to top-level repository areas as a conservative first-pass human boundary. Subsystems then decompose into responsibility slices and file leaves so a large branch like Visual App / Explorer does not stop before concrete editable files.

The deterministic architecture pass adds a runtime/dataflow layer beneath `project.architecture`. It uses package manifests, npm workspace metadata, bin commands, package entrypoints, UI file paths, API/route paths, server imports, local non-JS dependency edges, and the resolved import graph to create evidence-backed nodes such as CLI surface, core engine, scanner/tree/context pipeline, visual app API, visual app UI, runtime dataflow, and package distribution.

The architecture pass is intentionally evidence-limited. It can identify runtime boundaries that are visible in manifests, paths, resolved imports, local package metadata, TypeScript path aliases, supported bundler aliases, and conservative Python/Rust/Go/docs edges represented in the import graph. It does not infer hidden business semantics, dynamic routes assembled at runtime, unsupported alias plugins, environment-specific deployment topology, or complete user journeys. When the evidence is broad, architecture nodes should be read as deterministic ownership and dependency groupings rather than a complete design document.
...
Public CI runs deterministic Node checks on Ubuntu and Windows, plus Ubuntu-only audit, package-size, package smoke, and clean-checkout publish rehearsal jobs. It does not invoke the Windows-scoped loop scripts and does not require ignored runtime JSON or log files.
...
The browser app is local-first. It is served by the CLI and reads local project state through a local API. `atree serve` binds to `127.0.0.1` by default; LAN exposure requires an explicit non-loopback `--host` value and a bearer token supplied with `--token` or `ATREE_SERVE_TOKEN` because `/api/state` contains local project memory. Network clients must send `Authorization: Bearer <token>` for `/api/state`; the browser app can read the token from `#atree_token=<token>` or prompt after a 401 response. `atree serve --open` is an explicit convenience flag that launches the default browser after startup; plain `atree serve` only prints the URL. Projects that only install `@abstraction-tree/cli` can still scan, validate, and produce agent context packs without installing the UI.
