...
The architecture pass is intentionally evidence-limited. It can identify runtime boundaries that are visible in manifests, paths, resolved imports, and local package metadata, including TypeScript path aliases and supported bundler aliases represented in the import graph. It does not infer hidden business semantics, dynamic routes assembled at runtime, unsupported alias plugins, environment-specific deployment topology, or complete user journeys. When the evidence is broad, architecture nodes should be read as deterministic ownership and dependency groupings rather than a complete design document.
